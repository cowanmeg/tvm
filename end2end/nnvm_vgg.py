...
def get_network():
    ...
    network = sym.right_shift_channelwise(
        network, axis=3, name='binary_conv2d' + 'shift')
    network = sym.bitserial_conv2d(
        data=network,
        kernel_size=(3, 3),
        channels=256,
    ...
    network = sym.right_shift_channelwise(
        network, axis=3, name='binary_conv2d_1' + 'shift')
    network = sym.bitserial_conv2d(
        data=network,
        kernel_size=(3, 3),
        channels=256,
    ...
    network = sym.right_shift_channelwise(
        network, axis=3, name='binary_conv2d_2' + 'shift')
    network = sym.max_pool2d(
        network,
    ...
    network = network + r
    network = sym.clip_channelwise(
        network, axis=0, name='binary_dense' + 'clip')
    network = sym.right_shift_channelwise(
        network, axis=0, name='binary_dense' + 'shift')
    network = sym.flatten(data=network)
    network = sym.bitserial_dense(
        data=network,
        units=4096,
    ...
    network = network + r
    network = sym.clip_channelwise(
        network, axis=0, name='binary_dense_1' + 'clip')
    network = sym.right_shift_channelwise(
        network, axis=0, name='binary_dense_1' + 'shift')
    network = sym.flatten(data=network)
    network = sym.bitserial_dense(
        data=network,
        units=1000,
    ...
    network = network + r
    network = sym.clip_channelwise(
        network, axis=0, name='binary_dense_2' + 'clip')
    network = sym.right_shift_channelwise(
        network, axis=0, name='binary_dense_2' + 'shift')
    ...
    loaded_params = bytearray(
        open(os.path.join(d, "rasp_vggnet.params"), "rb").read())
...
def run(data, ctx):
    ...
    ftimer = module.module.time_evaluator(
        "run", ctx, number=REPEATS, repeat=num_iter)
